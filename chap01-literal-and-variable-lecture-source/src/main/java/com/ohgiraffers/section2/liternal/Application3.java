...
public class Application3 {
    public static void main(String[] args) {
         ...
         * 또한, 규칙에 맞는 올바른 병수명을 짓는 것이 좋은프로그래밍을 하는첫단추이다.
         * */

        /*
         * 변수의 명명 규칙
         * 1. 컴파일 에러를 발생시키는 규칙
         *   1-1. 동일한 범위 내에서 동일한 변수명을가질 수 있다.
         *   1-2. 예약어는 사용이 불가능하다.
         *       예약어? : 자바에서 미리 사용하겠다고, 컴파일러와 약속한키워드들을 의미한다.
         *   1-3. 변수명은 대문자와 소문자를 구분합니다.
         *   1-4. 변수명은 숫자로 시작 할 수 없다. (시작만 할 수 없고, 숫자를 섞는건 가능하다.
         *   1-5. 특수 기호는 '_'와 '$'만 사용 가능하다. (공백을 사용할 수 없습니다.)
         *
         * 2. 컴파일 에러를 발생시키지는 않지만개발자들끼리의 규칙(암묵적 규칙)
         *   2-1. 변수명의 길이 제한은 없다.
         *   2-2. 변수명이 합성어로 이루어진 경우, 첫단어는 소문자, 두번쨰 시작단어부터는 대문자로 시작한다.
         ...
         *   2-8. boolean 형은 의문문으로 가급적 긍정의형태로 네이밍을 합니다.
         *
         * */
        ...
        //int 1age = 20; // 숫자로 시작하기때문에 컴파일 에러가 발생한다.
        int age1 = 20; // 숫자가 맨앞에 있지 않으면 섞여서 사용 가능하다.

        //int a ge; // 사용가능한 특수 문자 외에는 사용이 불가능 하다.
        //int a@ge;
        int _a_g_e = 20; // 언더스코어(_) 사용 가능하다. 처음 중간 마지막 상관 없음.
        int $h$a$r$p$ = 20; // $ 사용 가능하다. 처음 중간 마지막 상관 없음.


        // 암묵적 약속
        // 변수의 길이에는 제한이 없지만, 적당히 하자.
        int awerawefsdfawefawefawefwf; // 이렇게 사용하지 말자.

        /*
        * 카멜케이스
        *
        * 합성어 일경우 첫단어는 모두 소문자, 두번째 시작 단어는 대문자로 시작한다.
        * 자바에서는 클래스명과 유일하게 대문자로 시작한다.
        **/
        int maxAge = 20;
        int minAge = 10;
